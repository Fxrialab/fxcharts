package fxrialab.controls.charts.layouts
{
	...
	public class FlowLayout extends LayoutBase {
		...
		override public function updateDisplayList(containerWidth:Number, containerHeight:Number):void {
			...
			for (var i:int = 0; i < count; i++) {
				...
				var newHeight:Number= y + elementHeight + _padding;
			...
			if (newHeight && layoutTarget.height != newHeight)
			{
				layoutTarget.height = newHeight;
				layoutTarget.setContentSize(containerWidth, newHeight);
				layoutTarget.invalidateSize();
				//layoutTarget.invalidateDisplayList();
			}
		//	layoutTarget.invalidateSize();
		//	trace('flow container process end');
		}
	}
}